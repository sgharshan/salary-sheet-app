...
export default function Modal({ open, onClose, title, children, footer }: Props) {
  ...
  return (
    <dialog
      ...
      className="modal-enter fixed inset-x-0 bottom-0 top-auto m-0 mx-auto max-h-[92dvh] w-full max-w-lg overflow-hidden open:flex open:flex-col rounded-t-3xl border border-white/10 bg-[#111] p-0 text-white shadow-2xl backdrop:bg-black/70 backdrop:backdrop-blur-sm sm:inset-0 sm:m-auto sm:max-h-[calc(100dvh-3rem)] sm:rounded-3xl"
    >
      <div className="mx-auto mt-3 h-1 w-10 shrink-0 rounded-full bg-white/15 sm:hidden" aria-hidden="true" />
      <div className="z-10 flex shrink-0 items-center justify-between gap-4 border-b border-white/[0.07] bg-[#111] px-5 py-4 sm:px-6">
      ...
      <div className="min-h-0 overflow-y-auto overscroll-contain p-5 pb-[max(1.5rem,env(safe-area-inset-bottom))] sm:p-6">{children}</div>
      {footer && <div className="shrink-0 border-t border-white/10 bg-[#111] px-5 pt-4 pb-[max(1rem,env(safe-area-inset-bottom))] sm:px-6">{footer}</div>}
    </dialog>
  )
}
